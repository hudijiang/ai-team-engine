...
AI Team Engine 是一个将**现代企业治理结构**（董事长 → CEO → 专业域员工）抽象为 Agent 层级协作关系的可视化平台。用户只需输入一句宏观商业/技术目标，系统即可自动完成：语义分析 → 团队组建 → 模型分配 → 依赖调度 → 并发执行 → 成果汇报的全链路自动化编排。
...
| **本地优先** | 默认可纯前端直连；也可启用本机单租户 Gateway 托管供应商密钥。完整控制平面仍未实现 |
...
| 项目 | 策略 |
|------|------|
| API Key 存储 | 直连供应商 Key 在 IndexedDB；Gateway 模式仅浏览器 Token，供应商 Key 在进程环境变量；localStorage bootstrap 不保存完整密钥 |
| 网络请求 | 直连时浏览器请求用户 URL；Gateway 时请求固定 Provider 注册表和白名单主机，按服务端 SSRF 边界防护 |
...
| 局限 | 说明 |
|------|------|
| 控制平面有限 | 已有本机 Gateway 代理与运行记录；无服务端任务执行器，执行仍随标签页结束 |
...
| 阶段 | 计划 |
|------|------|
| **P0 — 单租户控制平面** | 在现有 Gateway 上增加服务端执行器、事务运行库、审计与预算；并行增量拆 `ceoAgent.js` |
| **P0 — 主路径 E2E** | Playwright + 可选真实供应商烟雾测试 |
| **P1 — 工具运行时** | 沙箱 / 标准 MCP，替代简化 HTTP 桥 |
| **不做** | 对外 SaaS、多租户隔离、跨租户账单 |
