...
- ✅ **质量自动审核 (Self-Review)**
  - 每个阶段收尾时，CEO 自动调用 LLM 进行质量评审。
  - 首轮不达标会触发一次修订与复审，避免明显空泛或偏题的交付物直接进入报告。
...
- 🔧 **Agent 工具调用 (Tool Use)**
  - 已提供工具注册中心，内置计算器、时间查询、搜索等工具沙盘。
  - 支持自定义扩展；若需真实联网搜索或外部系统调用，仍需后端能力接入。

- 🔌 **MCP 协议支持**
  - 已提供 MCP Client 抽象层，可通过 MCP Server 对接企业内部系统。
  - 具体工具发现与调用链路需按部署环境接入。

- 📚 **知识库对接 (RAG)**
  - 已提供前端轻量 RAG 引擎，支持文档上传和关键词检索。
  - 检索结果可作为后续执行链集成的参考上下文。

- 🧩 **插件化 Agent 生态**
  - 已提供插件注册中心，内置电商和内容创作两个团队模板。
  - 支持自定义角色模板和工具链的注册、启用和禁用。

- 📈 **Agent 性能评估体系**
  - 按阶段追踪 QA 结果、LLM 响应耗时和 Token 规模，加权计算 0-100 评分。
  - Agent 排名面板可用于持续优化模型和 Prompt 配置。
...
仓库已提供 GitHub Actions CI，提交和 PR 会自动执行 `npm run check`。
...
1. **设置 API Key**：
   ...
   - 输入您持有的任意厂商 API Endpoint 和 API Key（完整配置持久化在浏览器 IndexedDB，`localStorage` 仅保留去敏 bootstrap 缓存）。
